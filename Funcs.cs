...
public class Funcs
{
    ...
    public static List<int> FreqArray(List<int> arr)
    {
        ...
        var counts = arr.GroupBy(t => t);//.ToLookup(t => t.Key, t => t.Count());
        List<int> freq = [];
        var enumerable = counts.ToList();
        for (var num = 0; num < arr.Count; num++)
        {
            var group = enumerable.FirstOrDefault(n => n.Key == num);
            freq.Add(group?.Count() ?? 0);
            Debug.WriteLine(string.Join(" ", freq.Select(f => $"{f}")), " ");
        }
        return freq;
    }
